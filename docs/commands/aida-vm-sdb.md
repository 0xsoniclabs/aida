...
## Overview
`aida-vm-sdb` is the Block Processing Manager. It functions similarly to a block processor, orchestrating the execution of transactions and blocks. It accepts transaction information in various formats, including [Substates](Terminology), JSON, and directly from a transaction feeder (see `txcontext` code package for details). It includes tools for iterating over substates, running Ethereum tests, and generating transactions for stress testing.
