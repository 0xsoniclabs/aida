...
var updateHashesCommand = cli.Command{
	Action:    updateHashesAction,
	Name:      "hashes",
	Usage:     "Downloads and stores block and state hashes into TargetDb for given range",
...
// updateHashesAction downloads and stores state hashes into Target for given range
func updateHashesAction(ctx *cli.Context) (finalErr error) {
	cfg, argErr := utils.NewConfig(ctx, utils.BlockRangeArgs)
	if argErr != nil {
		return argErr
	}

	log := logger.NewLogger(cfg.LogLevel, "UtilDb-Update-Hashes")
	...
	defer func() {
		finalErr = errors.Join(database.Close())
	}()
	...
	log.Infof("Updating hashes finished")
	return nil
}
...
//go:generate mockgen -source hashes.go -destination hashes_mock.go -package update
