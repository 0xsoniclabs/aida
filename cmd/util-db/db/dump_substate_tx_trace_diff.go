...
func substateDumpTxTraceDiffAction(ctx *cli.Context) error {
	...
	log := logger.NewLogger(cfg.LogLevel, "util-db dump substate tx trace diff")
	c, err := utils.GetClient(ctx.Context, cfg.ChainID, cfg.OperaDb+"/sonic.ipc", log)
	if err != nil {
		return fmt.Errorf("unable to connect to node; %w", err)
	}
	defer c.Close()

	getTransactionTraceFromRpc := func(block uint64, tx int) (map[string]interface{}, error) {
		return utils.GetTraceTransactionDiff(c, fmt.Sprintf("0x%x", block), fmt.Sprintf("0x%x", tx))
	}

	// run substate dump task
	taskPool := sdb.NewSubstateTaskPool("util-db dump substate tx trace diff", utildb.SubstateDumpTxTraceDiffFunc(getTransactionTraceFromRpc, log), cfg.First, cfg.Last, ctx)
