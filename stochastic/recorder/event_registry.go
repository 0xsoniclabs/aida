...
type EventRegistry struct {
	...
	contracts classifier.ArgClassifier[common.Address]

	// Storage-key access statistics
	keys classifier.ArgClassifier[common.Hash]

	// Storage-value access statistics
	values classifier.ArgClassifier[common.Hash]
...
func NewEventRegistry() EventRegistry {
	return EventRegistry{
		...
		contracts:    classifier.NewArgClassifier[common.Address](),
		keys:         classifier.NewArgClassifier[common.Hash](),
		values:       classifier.NewArgClassifier[common.Hash](),
		snapshotFreq: map[int]uint64{},
	}
}
...
func (r *EventRegistry) RegisterOp(op int) {
	if op < 0 || op >= operations.NumOps {
		log.Fatalf("invalid stochastic operation ID")
	}

	// classify simulation arguments
	addrClass := classifier.NoArgID
	keyClass := classifier.NoArgID
	valueClass := classifier.NoArgID
...
type EventRegistryJSON struct {
	...
	Contracts classifier.ArgClassifierJSON `json:"contractStats"`
	Keys      classifier.ArgClassifierJSON `json:"keyStats"`
	Values    classifier.ArgClassifierJSON `json:"valueSats"`
...
func (r *EventRegistry) NewEventRegistryJSON() EventRegistryJSON {
	...
	if len(r.snapshotFreq) > 0 {
		...
		simplifier := simplify.VisvalingamKeep(classifier.NumECDFPoints)
