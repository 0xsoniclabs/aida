...
func TestStateHash_ZeroHasSameStateHashAsOne(t *testing.T) {
	...
	err = StateAndBlockHashScraper(context.TODO(), chainid.TestnetChainID, "", database, 0, 1, log)
	if err != nil {
...
func TestStateHash_Log(t *testing.T) {
	...
	err = StateAndBlockHashScraper(context.TODO(), chainid.TestnetChainID, "", database, 9990, 10100, log)
	if err != nil {
		t.Fatalf("error scraping state hashes: %v", err)
	}
}
...
func TestStateHash_ZeroHasDifferentStateHashAfterHundredBlocks(t *testing.T) {
	...
	err = StateAndBlockHashScraper(context.TODO(), chainid.TestnetChainID, "", database, 0, 100, log)
	if err != nil {
		t.Fatalf("error scraping state hashes: %v", err)
	}
	err = database.Close()
...
func Test_getClient(t *testing.T) {
	...
	tests := []struct {
		...
		{"testGetClientRpcSonicMainnet", args{context.Background(), chainid.SonicMainnetChainID, ""}, &rpc.Client{}, false},
		{"testGetClientRpcOperaMainnet", args{context.Background(), chainid.MainnetChainID, ""}, &rpc.Client{}, false},
		{"testGetClientRpcTestnet", args{context.Background(), chainid.TestnetChainID, ""}, &rpc.Client{}, false},
		{"testGetClientIpcNonExistant", args{context.Background(), chainid.TestnetChainID, "/non-existant-path"}, nil, false},
...
func TestStateHash_GetClientIpcFail(t *testing.T) {
	...
	log := logger.NewLogger("info", "Test state hash")
	_, err := getClient(context.Background(), chainid.TestnetChainID, tmpIpcPath, log)
	if err == nil {
