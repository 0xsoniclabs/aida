...
func TestUtilsConfig_GetChainConfig(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("ChainID: %d", tc), func(t *testing.T) {
			...
			if tc == OperaMainnetID && chainConfig.BerlinBlock.Cmp(new(big.Int).SetUint64(37455223)) != 0 {
				t.Fatalf("Incorrect Berlin fork block on chainID: %d; Block number: %d, should be: %d", OperaMainnetID, chainConfig.BerlinBlock, 37455223)
			}

			if tc == OperaMainnetID && chainConfig.LondonBlock.Cmp(new(big.Int).SetUint64(37534833)) != 0 {
				t.Fatalf("Incorrect London fork block on chainID: %d; Block number: %d, should be: %d", OperaMainnetID, chainConfig.LondonBlock, 37534833)
			}

			if tc == OperaTestnetID && chainConfig.BerlinBlock.Cmp(new(big.Int).SetUint64(1559470)) != 0 {
				t.Fatalf("Incorrect Berlin fork block on chainID: %d; Block number: %d, should be: %d", OperaTestnetID, chainConfig.BerlinBlock, 1559470)
			}

			if tc == OperaTestnetID && chainConfig.LondonBlock.Cmp(new(big.Int).SetUint64(7513335)) != 0 {
				t.Fatalf("Incorrect London fork block on chainID: %d; Block number: %d, should be: %d", OperaTestnetID, chainConfig.LondonBlock, 7513335)
			}
		})
	}
}
...
func TestUtilsConfig_SetBlockRange(t *testing.T) {
	...
	first, last, err = SetBlockRange("OpeRa", "berlin", OperaMainnetID)
	if err != nil {
	...
	first, last, err = SetBlockRange("zero", "London", OperaTestnetID)
	if err != nil {
	...
	first, last, err = SetBlockRange("opera+23456", "London-100", OperaTestnetID)
	if err != nil {
	...
	first, last, err = SetBlockRange("berlin-1000", "LonDoN", OperaMainnetID)
	if err != nil {
	...
	first, last, err = SetBlockRange("first", "last", OperaMainnetID)
	if err != nil {
	...
	first, last, err = SetBlockRange("lastpatch", "last", OperaMainnetID)
	if err != nil {
...
func TestUtilsConfig_SetInvalidBlockRange(t *testing.T) {
	...
	_, _, err = SetBlockRange("1000", "0", OperaTestnetID)
	if err == nil {
		t.Fatalf("Failed to throw an error")
	}

	_, _, err = SetBlockRange("tokyo", "berlin", OperaMainnetID)
	if err == nil {
		t.Fatalf("Failed to throw an error")
	}

	_, _, err = SetBlockRange("tokyo", "berlin", OperaTestnetID)
	if err == nil {
		t.Fatalf("Failed to throw an error")
	}

	_, _, err = SetBlockRange("london-opera", "opera+london", OperaMainnetID)
	if err == nil {
		t.Fatalf("Failed to throw an error")
	}

	_, _, err = SetBlockRange("london-opera", "opera+london", OperaTestnetID)
	if err == nil {
		t.Fatalf("Failed to throw an error")
	}
}
...
func TestUtilsConfig_getMdBlockRange(t *testing.T) {
	...
	var (
		logLevel   = "NOTICE"
		firstBlock = KeywordBlocks[OperaMainnetID]["opera"]
...
func TestUtilsConfig_updateConfigBlockRangeBlockRange(t *testing.T) {
	...
	// prepare mock config
	cfg := &Config{AidaDb: "./test.db", LogLevel: logLevel, ChainID: OperaMainnetID}
