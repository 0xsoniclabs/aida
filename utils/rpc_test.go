...
const invalidChainID chainid.ChainID = -1
...
func TestSendRPCRequest_Positive(t *testing.T) {
	...
	for _, id := range maps.Keys(chainid.RealChainIDs) {
...
func TestRpc_FindEpochNumber(t *testing.T) {
	// case success
	output, err := FindEpochNumber(uint64(1234), chainid.MainnetChainID)
...
func TestRpc_FindHeadEpochNumber(t *testing.T) {
	// case success
	output, err := FindHeadEpochNumber(chainid.MainnetChainID)
	assert.NoError(t, err)
...
func TestRpc_getEpochByNumber(t *testing.T) {
	// case success
	output, err := getEpochByNumber("0x4e20", chainid.MainnetChainID)
	assert.NoError(t, err)
