...
func TestPrimeContext_SelfDestructAccounts(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		mockStateDb.EXPECT().EndTransaction().Return(nil)
		mockStateDb.EXPECT().EndBlock().Return(nil)
		mockStateDb.EXPECT().EndSyncPeriod().Return(nil)
		mockStateDb.EXPECT().Exist(gomock.Any()).Return(true).Times(2)
		mockStateDb.EXPECT().SelfDestruct(gomock.Any()).Return(*uint256.NewInt(99)).Times(2)
		mockLogger.EXPECT().Debugf(gomock.Any(), gomock.Any()).MinTimes(1)
		mockLogger.EXPECT().Infof(gomock.Any(), gomock.Any()).MinTimes(1)
	...
	t.Run("error end sync period", func(t *testing.T) {
		...
		mockStateDb.EXPECT().EndTransaction().Return(nil)
		mockStateDb.EXPECT().EndBlock().Return(nil)
		mockStateDb.EXPECT().EndSyncPeriod().Return(mockError)
		mockStateDb.EXPECT().Exist(gomock.Any()).Return(true).Times(2)
		mockStateDb.EXPECT().SelfDestruct(gomock.Any()).Return(*uint256.NewInt(99)).Times(2)
		mockLogger.EXPECT().Debugf(gomock.Any(), gomock.Any()).MinTimes(1)
		err := prime.SelfDestructAccounts(mockStateDb, []substatetypes.Address{
