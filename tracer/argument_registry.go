...
// ArgumentContext counts events and counts transition for the Markov-Process.
...
func NewArgumentContext() ArgumentContext {
...
func (r *ArgumentContext) RegisterOp(op int) {
	if op < 0 || op >= NumOps {
...
func (r *ArgumentContext) RegisterAddressOp(op int, address *common.Address) {
	// check ID
	if op < 0 || op >= NumOps {
		log.Fatalf("invalid stochastic operation ID")
	}
...
func (r *ArgumentContext) RegisterKeyOp(op int, address *common.Address, key *common.Hash) {
...
func (r *ArgumentContext) RegisterValueOp(op int, address *common.Address, key *common.Hash, value *common.Hash) {
	// check operation range
	if op < 0 || op >= NumOps {
		log.Fatalf("invalid stochastic operation ID")
	}
...
func (r *ArgumentContext) updateFreq(op int, addr int, key int, value int) {
...
func (r *ArgumentContext) RegisterSnapshotDelta(delta int) {
	r.snapshotFreq[delta]++
}

// WriteJSON writes an event registry in JSON format.
func (r *ArgumentContext) WriteJSON(filename string) error {
	f, fErr := os.Create(filename)
	if fErr != nil {
		return fmt.Errorf("cannot open JSON file; %v", fErr)
	}
	defer f.Close()
	jOut, jErr := json.MarshalIndent(r.NewArgumentContextJSON(), "", "    ")
	if jErr != nil {
...
// ArgumentContextJSON is the JSON struct for an event registry.
type ArgumentContextJSON struct {
...
// NewArgumentContext produces the JSON output for an event registry.
func (r *ArgumentContext) NewArgumentContextJSON() ArgumentContextJSON {
...
func ReadEvents(filename string) (*ArgumentContextJSON, error) {
	...
	var eventsJSON ArgumentContextJSON
