...
func FindDbHashOnline(chainId config.ChainID, log logger.Logger, md *metadata.AidaDbMetadata) ([]byte, error) {
	var url string

	if chainId == config.SonicMainnetChainID {
		url = config.AidaDbRepositorySonicUrl
	} else if chainId == config.MainnetChainID {
		url = config.AidaDbRepositoryOperaUrl
	} else if chainId == config.TestnetChainID {
		url = config.AidaDbRepositoryTestnetUrl
	}

	log.Noticef("looking for db-hash online on %v", url)
	patches, err := metadata.DownloadPatchesJson()
	if err != nil {
		return nil, err
	}
	...
	md.FirstBlock, md.LastBlock, ok = metadata.FindBlockRangeInSubstate(db.MakeDefaultSubstateDBFromBaseDB(md.Db))
	if !ok {
