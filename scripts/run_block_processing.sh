...
./build/aida-vm-sdb substate --profile-blocks --profile-db "$outputdir/profile.db" --aida-db $aidadbpath --db-impl $dbimpl --db-variant $dbvariant --vm-impl=$vmimpl --db-tmp $tmpdir --carmen-schema $carmenschema --update-buffer-size $buffersize --substate-encoding $substateencoding $startblock $endblock
