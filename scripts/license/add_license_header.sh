...
ignore_files=('carmen/*' 'sonic/*' 'tosca/* *mock')
...
# Updates the copyright year and holder in cli.App{} definitions under the cmd/ directory
update_cli_app_copyright() {
    local target_dir="$root_dir/cmd"
    local year=2025
    local holder="Sonic Labs"
    local pattern='Copyright:[[:space:]]*"(c)[^"]*"'
    local replacement="Copyright: \"(c) $year $holder\""

    find "$target_dir" -type f -name "*.go" | while read -r f; do
        if grep -q 'cli\.App{' "$f"; then
            if $check_only; then
                if grep -q 'Copyright: "(c) 2022 Fantom Foundation"' "$f"; then
                    echo "Obsolete cli.App copyright in: $f"
                    result=1
                fi
            else
                sed -i -E "s/$pattern/$replacement/" "$f"
            fi
        fi
    done
}
...
echo "Updating copyright in cli.App definitions"
update_cli_app_copyright
exit $result