...
TARGET_DB_FOR_SCRAPE="${AIDA_DB_BASE_NAME}-${FIRST_BLOCK}-${LAST_BLOCK}"
...
echo "${UTIL_DB_BIN} scrape --target-db ${TARGET_DB_FOR_SCRAPE} --db ${SONIC_PATH} --log debug ${FIRST_BLOCK} ${LAST_BLOCK}"
"${UTIL_DB_BIN}" scrape \
...
"${BASH_SOURCE%/*}/metadata_generator.sh" "${UTIL_DB_BIN}" "${AIDA_DB_BASE_NAME}" "${FIRST_BLOCK}" "${LAST_BLOCK}" "${FIRST_EPOCH}" "${LAST_EPOCH}"

echo "Executing generate-db-hash command:"
echo "${UTIL_DB_BIN} generate-db-hash --aida-db ${AIDA_DB_BASE_NAME}"
...
echo "Generating patch file for AIDA database: ${AIDA_DB_BASE_NAME}.tar.gz"
# Generate the patch file as a compressed tar.gz archive
tar vfc - "${AIDA_DB_BASE_NAME}" | pigz > "${AIDA_DB_BASE_NAME}.tar.gz"
