...
func (e *stateDbCorrector) loadStateFromException(scope correctorScope, tx int) (substate.WorldState, error) {
	var ws substate.WorldState
	switch {
	case scope == preBlock:
		ws = *e.currentException.Data.PreBlock
	case scope == postBlock:
		ws = *e.currentException.Data.PostBlock
	case scope == preTransaction:
		if _, ok := e.currentException.Data.Transactions[tx]; !ok {
			return nil, nil
		}
		ws = *e.currentException.Data.Transactions[tx].PreTransaction
	case scope == postTransaction:
		if _, ok := e.currentException.Data.Transactions[tx]; !ok {
			return nil, nil
		}
		ws = *e.currentException.Data.Transactions[tx].PostTransaction
	default:
